...
// Базовий абстрактний класс треба використовувати у випадку якщо вам потрібно реалізувати поліморфізм, тобто варіативність реалізації методів для клієнстького коду. У випадку компонентів немає клієнського коду, який би користувався цими методами, це є неправильно. У вашому випадку ви лише вирішуєте задачу повторного використання коду, ціною значних обмежень або перегрузкою базового компоненту зайвим кодом. Так чи інакше ви або наробите багато базових класів, або ваш базовий клас перетвориться в звалище коду. У випадку компонентів краще розповсюджувати повторюваний код за допомогою сервісів.
@Directive()
  ...
  implements OnInit
{
  ...
  // Оголошення типу з ініціалізацією скіпається лише для примитивів. Для об'єктів треба явно вказувати тип. Тут і далі багато таких моментів.
  ...
  ngOnInit(): void {
    ...
    // 1. Для loader'а краще завести примитив який буде встановлювати флаг в true/false. Окрім того флаг має бути не boolean а number, для одночасного користування декількома observables. Ділюся прикладом:
    ...
    */
    ...
    // 2. Кожен виклик api має супроводжуватися обробкою помилок. Для цього потрібно мати власний клас для помилок, дочірній від Error і кидати цей ApiError з api сервісів. Може бути багато різних апі, з різними стандартами json'у для цих помилок. Для фронтенду важливо привести все до єдиного стандарту, до ApiError extends Error. Також треба завести примитив який буде ловити ApiError і показувати повідомлення користувачу:
    /*
      this.dataUpdateRequested$
      .asObservable()
      .pipe(
    ...
    */
    ...
    // 4. Компоненти не мають загружати дані, це прерогатива компонентів-контейнерів або data-resolver'ів. Компонент контайнер це - компонент з дуже простим view який лише читає path, грузить дані та відає дані дочірнім плоским компонентам які їх відображають
